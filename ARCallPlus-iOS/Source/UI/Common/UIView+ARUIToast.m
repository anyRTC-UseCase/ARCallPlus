...
// Positions
NSString * ARUICSToastPositionTop                       = @"ARUICSToastPositionTop";
NSString * ARUICSToastPositionCenter                    = @"ARUICSToastPositionCenter";
NSString * ARUICSToastPositionBottom                    = @"ARUICSToastPositionBottom";

// Keys for values associated with toast views
static const NSString * ARUICSToastTimerKey             = @"ARUICSToastTimerKey";
static const NSString * ARUICSToastDurationKey          = @"ARUICSToastDurationKey";
static const NSString * ARUICSToastPositionKey          = @"ARUICSToastPositionKey";
static const NSString * ARUICSToastCompletionKey        = @"ARUICSToastCompletionKey";

// Keys for values associated with self
static const NSString * ARUICSToastActiveKey            = @"ARUICSToastActiveKey";
static const NSString * ARUICSToastActivityViewKey      = @"ARUICSToastActivityViewKey";
static const NSString * ARUICSToastQueueKey             = @"ARUICSToastQueueKey";

@interface UIView (ARUIToastPrivate)
...
@implementation UIView (ARUIToast)

#pragma mark - Make Toast Methods

- (void)makeToast:(NSString *)message {
    [self makeToast:message duration:[ARUICSToastManager defaultDuration] position:[ARUICSToastManager defaultPosition] style:nil];
}

- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration {
    [self makeToast:message duration:duration position:[ARUICSToastManager defaultPosition] style:nil];
}
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position style:(ARUICSToastStyle *)style {
    UIView *toast = [self toastViewForMessage:message title:nil image:nil style:style];
    [self showToast:toast duration:duration position:position completion:nil];
}

- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position title:(NSString *)title image:(UIImage *)image style:(ARUICSToastStyle *)style completion:(void(^)(BOOL didTap))completion {
...
- (void)showToast:(UIView *)toast {
    [self showToast:toast duration:[ARUICSToastManager defaultDuration] position:[ARUICSToastManager defaultPosition] completion:nil];
}
...
- (void)showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)position completion:(void(^)(BOOL didTap))completion {
    ...
    objc_setAssociatedObject(toast, &ARUICSToastCompletionKey, completion, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    
    if ([ARUICSToastManager isQueueEnabled] && [self.cs_activeToasts count] > 0) {
        // we're about to queue this toast view so we need to store the duration and position as well
        objc_setAssociatedObject(toast, &ARUICSToastDurationKey, @(duration), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        objc_setAssociatedObject(toast, &ARUICSToastPositionKey, position, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)cs_showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)position {
    ...
    if ([ARUICSToastManager isTapToDismissEnabled]) {
    ...
    [UIView animateWithDuration:[[ARUICSToastManager sharedStyle] fadeDuration]
                          delay:0.0
                     ...
                     animations:^{
                         ...
                         objc_setAssociatedObject(toast, &ARUICSToastTimerKey, timer, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
                     }];
}
...
- (void)cs_hideToast:(UIView *)toast fromTap:(BOOL)fromTap {
    NSTimer *timer = (NSTimer *)objc_getAssociatedObject(toast, &ARUICSToastTimerKey);
    [timer invalidate];
    ...
    [UIView animateWithDuration:[[ARUICSToastManager sharedStyle] fadeDuration]
                     ...
                     animations:^{
                         ...
                         void (^completion)(BOOL didTap) = objc_getAssociatedObject(toast, &ARUICSToastCompletionKey);
                         if (completion) {
                             completion(fromTap);
                         }
                         ...
                         if ([self.cs_toastQueue count] > 0) {
                             ...
                             NSTimeInterval duration = [objc_getAssociatedObject(nextToast, &ARUICSToastDurationKey) doubleValue];
                             id position = objc_getAssociatedObject(nextToast, &ARUICSToastPositionKey);
...
- (UIView *)toastViewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image style:(ARUICSToastStyle *)style {
    // sanity
    if (message == nil && title == nil && image == nil) return nil;
    
    // default to the shared style
    if (style == nil) {
        style = [ARUICSToastManager sharedStyle];
    }
...
- (NSMutableArray *)cs_activeToasts {
    NSMutableArray *cs_activeToasts = objc_getAssociatedObject(self, &ARUICSToastActiveKey);
    if (cs_activeToasts == nil) {
        cs_activeToasts = [[NSMutableArray alloc] init];
        objc_setAssociatedObject(self, &ARUICSToastActiveKey, cs_activeToasts, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    return cs_activeToasts;
}

- (NSMutableArray *)cs_toastQueue {
    NSMutableArray *cs_toastQueue = objc_getAssociatedObject(self, &ARUICSToastQueueKey);
    if (cs_toastQueue == nil) {
        cs_toastQueue = [[NSMutableArray alloc] init];
        objc_setAssociatedObject(self, &ARUICSToastQueueKey, cs_toastQueue, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
...
- (void)makeToastActivity:(id)position {
    // sanity
    UIView *existingActivityView = (UIView *)objc_getAssociatedObject(self, &ARUICSToastActivityViewKey);
    if (existingActivityView != nil) return;
    
    ARUICSToastStyle *style = [ARUICSToastManager sharedStyle];
    ...
    objc_setAssociatedObject (self, &ARUICSToastActivityViewKey, activityView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    ...
    if (existingActivityView != nil) {
        [UIView animateWithDuration:[[ARUICSToastManager sharedStyle] fadeDuration]
                         ...
                         animations:^{
                             ...
                             objc_setAssociatedObject (self, &ARUICSToastActivityViewKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
                         }];
    }
}
...
- (CGPoint)cs_centerPointForPosition:(id)point withToast:(UIView *)toast {
    ...
    if([point isKindOfClass:[NSString class]]) {
        if([point caseInsensitiveCompare:ARUICSToastPositionTop] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width / 2.0, (toast.frame.size.height / 2.0) + topPadding);
        } else if([point caseInsensitiveCompare:ARUICSToastPositionCenter] == NSOrderedSame) {
...
@implementation ARUICSToastStyle
...
@interface ARUICSToastManager ()

@property (strong, nonatomic) ARUICSToastStyle *sharedStyle;
...
@implementation ARUICSToastManager

#pragma mark - Constructors

+ (instancetype)sharedManager {
    static ARUICSToastManager *_sharedManager = nil;
...
- (instancetype)init {
    self = [super init];
    if (self) {
        self.sharedStyle = [[ARUICSToastStyle alloc] initWithDefaultStyle];
        self.tapToDismissEnabled = YES;
        self.queueEnabled = NO;
        self.defaultDuration = 3.0;
        self.defaultPosition = ARUICSToastPositionBottom;
    }
    return self;
}
...
+ (void)setSharedStyle:(ARUICSToastStyle *)sharedStyle {
    [[self sharedManager] setSharedStyle:sharedStyle];
}

+ (ARUICSToastStyle *)sharedStyle {
